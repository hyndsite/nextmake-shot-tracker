...
export const BASE_METRIC_OPTIONS = [
  ...
  // ------------- Metric calculators (game events) ------------------------
  ...
  // ------------- practice metrics (reuse game logic for now) -------------
  
  /**
   * For now we assume practice_entries mirror the shooting-related fields
   * of game_events (type, made, is_three, zone_id, shot_type, pressured, ts).
   * That lets us reuse the same aggregation + metric logic.
   *
   * If practice_entries diverge later, we can introduce a dedicated
   * aggregatePracticeEntries() and swap it in here without changing callers.
   */
  export function computePracticeMetricValue(
    ...
    return computeGameMetricValue(metricKey, entries, { startDate, endDate, zoneId })
  }
